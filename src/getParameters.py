...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [7, 7, 1, 10],
                         initializer=tf.contrib.layers.xavier_initializer(
                             seed=0, dtype=tf.float32))
    W2 = tf.get_variable("W2", [7, 7, 10, 20],
                         initializer=tf.contrib.layers.xavier_initializer(
                             seed=0, dtype=tf.float32))
...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    # number of mini batches of size mini_batch_size in a partition
    num_complete_minibatches = math.floor(m / mini_batch_size)
    for k in range(0, num_complete_minibatches):

        mini_batch_X = shuffled_X[:, mini_batch_size * k:mini_batch_size *
                                  (k + 1)]
        mini_batch_Y = shuffled_Y[:, mini_batch_size * k:mini_batch_size *
                                  (k + 1)]
    ...
    if m % mini_batch_size != 0:
        ...
        mini_batch_X = shuffled_X[:,
                                  mini_batch_size * num_complete_minibatches:m]
...
def forward_propagation(X, parameters):
    ...
    conv_filters = {"conv1_s": 1, "conv2_s": 1}

    pool_filters = {"pool1_s": 2, "pool1_f": 2, "pool2_s": 2, "pool2_f": 2}
    ...
    Z1 = tf.nn.conv2d(
        X,
        W1,
        strides=[1, conv_filters["conv1_s"], conv_filters["conv1_s"], 1],
        padding='VALID')  # + b1;
    ...
    P1 = tf.nn.max_pool(
        A1,
        ksize=[1, pool_filters["pool1_f"], pool_filters["pool1_f"], 1],
        strides=[1, pool_filters["pool1_s"], pool_filters["pool1_s"], 1],
        padding='VALID')
    ...
    Z2 = tf.nn.conv2d(
        P1,
        W2,
        strides=[1, conv_filters["conv2_s"], conv_filters["conv2_s"], 1],
        padding='VALID')  # + b2;
    ...
    P2 = tf.nn.max_pool(
        A2,
        ksize=[1, pool_filters["pool2_f"], pool_filters["pool2_f"], 1],
        strides=[1, pool_filters["pool2_s"], pool_filters["pool2_s"], 1],
        padding='VALID')
    ...
    with tf.variable_scope("LogReg"):
        Z4 = tf.contrib.layers.fully_connected(P2,
                                               84,
                                               activation_fn=tf.nn.relu,
                                               scope='fc2')
        Z5 = tf.contrib.layers.fully_connected(Z4,
                                               10,
                                               activation_fn=None,
                                               scope='fc3')
