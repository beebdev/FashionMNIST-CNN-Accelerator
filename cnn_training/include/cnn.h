...
static void save_weights(layer_t *layer, std::ofstream &outfile) {
	switch (layer->type) {
	case layer_type::conv:
		((conv_layer_t *) layer)->save_weights(outfile);
		return;
	case layer_type::relu:
		((relu_layer_t *) layer)->save_weights(outfile);
		return;
	case layer_type::fc:
		((fc_layer_t *) layer)->save_weights(outfile);
		return;
	case layer_type::pool:
		((pool_layer_t *) layer)->save_weights(outfile);
		return;
	case layer_type::dropout_layer:
		((dropout_layer_t *) layer)->save_weights(outfile);
		return;
	default:
		assert(false);
	}
