...
#pragma pack(push, 1)
struct conv_layer_t {
	layer_type type = layer_type::conv;					// Type of layer
	tensor_t<float> grads_in;
	tensor_t<float> in;									// Input data
	tensor_t<float> out;								// Output data
	std::vector<tensor_t<float>> filters;				// Filters
	std::vector<tensor_t<gradient_t>> filter_grads;		// Filter gradiants
	uint16_t stride;									// #strides
	uint16_t extend_filter;								// Padding

	conv_layer_t(uint16_t stride, uint16_t extend_filter, uint16_t number_filters, tdsize in_size)
		:
		grads_in(in_size.x, in_size.y, in_size.z),
		in(in_size.x, in_size.y, in_size.z),
		out((in_size.x - extend_filter) / stride + 1,
